...
@NamedQueries({
    ...
    @NamedQuery(name = "Listing.findWithPendingBookingsByCustomerID", query = "select l from Listing l where l.id  in (select b.listingId.id from Booking b where b.bookingStatus = 0 and b.userId.id = :x) and l.active=1"),
    @NamedQuery(name = "Listing.findWithAcceptedBookingsByCustomerID", query = "select l from Listing l where l.id  in (select b.listingId.id from Booking b where b.bookingStatus = 1 and b.userId.id = :x) and l.active=1"),
...
//    @NamedQuery(name = "Listing.findWithPreviousBookingsByHostID", query = "select l from Listing l where l.id  in (select b.listingId.id from Booking b where b.bookingStatus = 1 and b.listingId.userId.id =  :x) and l.active=1"),//    @NamedQuery(name = "Message.findReceivedByUserId", query = "SELECT m FROM Message m WHERE m.userIdTo.id = :x"),
